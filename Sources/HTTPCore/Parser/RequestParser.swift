...
struct RequestParserContext {
    var method: HTTPCore.Method! = nil
    var url: URL! = nil
...
public final class RequestParser {
    ...
    public func parse(_ data: Data) throws -> Request? {
        let pointer = data.withUnsafeBytes { (bytes: UnsafePointer<Int8>) in
            UnsafeRawPointer(bytes).assumingMemoryBound(to: Int8.self)
        }
        let bytesParsed = http_parser_execute(&parser, &parserSettings, pointer, data.count)
    ...
    func parserProcess(state: RequestParserState, data: UnsafeBufferPointer<Int8>? = nil) {
        if parserState != state {
            ...
            case .headersComplete:
                parserContext.method = HTTPCore.Method(code: Int(parser.method))
                parserContext.version = Version(major: Int(parser.http_major), minor: Int(parser.http_minor))
                parserContext.currentHeaderField = nil

            case .body:
                parserContext.body = Data(parserBuffer)
            }
